...
class BamRead {
 ...
 public:
  ...
  inline bool ReverseFlag() { return (b->core.flag&BAM_FREVERSE) != 0; }

  /** Mate of BamRead is aligned on reverse strand */
  inline bool MateReverseFlag() { return (b->core.flag&BAM_FMREVERSE) != 0; }
  
    /** Get the alignment position */
    inline int32_t Position() { return b->core.pos; }

    /** Get the alignment position of the mate */
    inline int32_t MatePosition() { return b->core.mpos; }

    /** Get the chromosome ID of the read */
    inline int32_t ChrID() { return b->core.tid; }

    /** Get the chrosome ID of the mate read */
    inline int32_t MateChrID() { return b->core.mtid; }

    /** Check if this read is mapped*/
    inline bool MappedFlag() { return !(b->core.flag&BAM_FUNMAP); }

    /** Check if this reads mate is mapped*/
    inline bool MateMappedFlag() { return !(b->core.flag&BAM_FMUNMAP); }
	
    /** Get the mapping quality */
    inline int32_t MapQuality() { return b->core.qual; }

    /** Get the number of cigar fields */
    inline int32_t CigarSize() { return b->core.n_cigar; }

    /** Check if this read is first in pair */
    inline bool FirstFlag() { return (b->core.flag&BAM_FREAD1); }

    /** Get the qname of this read as a string */
    inline std::string Qname() { return std::string(bam_get_qname(b)); }

    /** Get the qname of this read as a char array */
    inline char* QnameChar() { return bam_get_qname(b); }

    /** Get the full alignment flag for this read */
    inline int32_t AlignmentFlag() { return b->core.flag; }
    ...
    inline int32_t Length() { return b->core.l_qseq; }
     ...
     */
    inline std::string GetZTag(const std::string& tag) {
